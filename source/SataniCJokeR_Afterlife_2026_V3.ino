...
void drawManualRing(uint8_t headLed, bool blue = false) {
  uint32_t colorDim    = blue ? PackLEDs.Color(0, 0, 60)  : PackLEDs.Color(60, 0, 0);
  uint32_t colorMedium = blue ? PackLEDs.Color(0, 0, 140) : PackLEDs.Color(140, 0, 0);
  uint32_t colorBright = blue ? PackLEDs.Color(0, 0, 255) : PackLEDs.Color(255, 0, 0);
...
void runManualIdlePack(bool blue = false) {
  ...
  drawManualRing(ringHead, blue);    // draw the Cyclotron precisely on top afterwards
  PackLEDs.show();
}
...
/************************* Music: simulated PowerCell VU meter *************************/
// Software-only visual effect. It does NOT sample the DFPlayer's audio and is
// therefore not synchronized to the actual music or its volume.
// D2 pixels 0..39 remain the blue manual Cyclotron; 40..54 are the VU bar.
const uint8_t MUSIC_VU_START_LED = 40;
const uint8_t MUSIC_VU_LED_COUNT = 15;
uint8_t musicVuLevel = 0;
uint8_t musicVuTarget = 0;
uint8_t musicVuPeak = 0;
uint16_t musicVuRandom = 0xACE1U;
unsigned long musicVuLastTargetAt = 0;
unsigned long musicVuLastStepAt = 0;
unsigned long musicVuPeakSetAt = 0;
unsigned long musicVuLastPeakDropAt = 0;

void resetMusicVu(unsigned long now) {
  musicVuLevel = 0;
  musicVuTarget = 8;
  musicVuPeak = 0;
  // Different starting phase on each entry; no extra sensor or analog input.
  musicVuRandom ^= (uint16_t)micros();
  musicVuLastTargetAt = now;
  musicVuLastStepAt = now;
  musicVuPeakSetAt = now;
  musicVuLastPeakDropAt = now;
}

void updateMusicVu(unsigned long now) {
  // Change the simulated signal at slightly irregular intervals (110..237 ms).
  if ((unsigned long)(now - musicVuLastTargetAt) >=
      (110UL + (unsigned long)(musicVuRandom & 0x7FU))) {
    musicVuLastTargetAt = now;
    musicVuRandom = (uint16_t)(musicVuRandom * 109U + 89U);
    musicVuTarget = (uint8_t)((musicVuRandom >> 8) & 0x0FU); // 0..15
  }

  // Quick attack, slower release: the bar rises rapidly and falls smoothly.
  if ((unsigned long)(now - musicVuLastStepAt) >= 45UL) {
    musicVuLastStepAt = now;
    if (musicVuLevel < musicVuTarget) {
      uint8_t remaining = musicVuTarget - musicVuLevel;
      musicVuLevel += (remaining >= 3U) ? 3U : remaining;
    } else if (musicVuLevel > musicVuTarget) {
      musicVuLevel--;
    }

    // A short-lived peak marker above the current bar.
    if (musicVuLevel > musicVuPeak) {
      musicVuPeak = musicVuLevel;
      musicVuPeakSetAt = now;
      musicVuLastPeakDropAt = now;
    } else if (musicVuPeak > 0 &&
               (unsigned long)(now - musicVuPeakSetAt) >= 240UL &&
               (unsigned long)(now - musicVuLastPeakDropAt) >= 100UL) {
      musicVuPeak--;
      musicVuLastPeakDropAt = now;
    }
  }

  // Preserve the PowerCell's blue color, with a lighter leading edge.
  const uint32_t fillColor = PackLEDs.Color(0, 0, 210);
  const uint32_t tipColor  = PackLEDs.Color(70, 110, 255);
  const uint32_t peakColor = PackLEDs.Color(15, 35, 180);
  for (uint8_t i = 0; i < MUSIC_VU_LED_COUNT; i++) {
    uint32_t pixel = 0;
    if (i < musicVuLevel) pixel = (i == musicVuLevel - 1U) ? tipColor : fillColor;
    PackLEDs.setPixelColor(MUSIC_VU_START_LED + i, pixel);
  }
  if (musicVuPeak > musicVuLevel && musicVuPeak <= MUSIC_VU_LED_COUNT) {
    PackLEDs.setPixelColor(MUSIC_VU_START_LED + musicVuPeak - 1U, peakColor);
  }
}

void runMusicVisuals(unsigned long now) {
  // Do not call PackLEDs.Update() here: the normal PowerCell pattern would
  // overwrite the simulated meter. Everything else retains FIX20b behavior.
  stepManualRingAtRPM(300.0f);
  drawManualRing(ringHead, true);
  updateMusicVu(now);
  PackLEDs.show(); // One refresh for the blue Cyclotron and the VU PowerCell.
}
...
// Music from /01: same Idle 1 pack and bargraph animations, blue Cyclotron.
// Called once when entering music from Standby; does not change DFPlayer playback.
void enterMusicVisuals() {
  PackLEDs.AL_Fire(false);
  startManualIdle();
  PackLEDs.setBrightness(80); // Music-only brightness; normal Idle stays at 10.
...
void updatePackAndBarGraph(unsigned long now) {
  switch (state) {
    ...
    case ST_MUSIC:
      runMusicVisuals(now);          // Blue Cyclotron + simulated PowerCell VU
      BarGraph.sequencePackOn(now); // Same Bargraph sequence as Idle 1
      break;
